...
def getReading(txt):
    endingPosition = txt.index("(<a href=\"https://szentiras.hu/RUF")
    return txt[0: endingPosition]

def getReadingLink(txt):
    startingPosition = txt.index("(<a href=")+10
    endingPosition = txt.index("\" target=\"_blank\">")
    return txt[startingPosition: endingPosition]

def getReadingShort(txt):
    startingPosition = txt.index("target=\"_blank\">")+16
    endingPosition = txt.index("</a>)")
    return txt[startingPosition: endingPosition]

dailyReadings = []

with open('../dailyReadings_v2.csv', encoding='utf-8') as csv_file:
